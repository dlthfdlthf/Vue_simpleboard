...
export default {
  data() {
    return {
      newinfo: {
        newtitle: this.$attrs.test.item.Title,
        newcontext: this.$attrs.test.item.context,
        Enrollment_date: this.$attrs.test.item.Enrollment_date,
        Id_name: "lutes123"
      }
    };
  },
    ...
    delete_contents: function(event) {
      if (confirm("이슈사항을 삭제하겠습니까?")) {
        this.$http({
          ...
          data: {
            // post, put, delete에 사용
            Title: this.$attrs.test.item.Title,
            context: this.$attrs.test.item.context,
            Enrollment_date: this.newinfo.Enrollment_date,
            Id_name: this.newinfo.Id_name,
            // post(등록), put(수정)의 경우만 쓰임
            // resMsg: "this is put test"
